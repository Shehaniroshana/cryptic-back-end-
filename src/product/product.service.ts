...
export class ProductService {
    ...
    try {
      ...
      if (
        await this.productRepo.findOne({
          where: { seller: { id: product.seller }, name: product.name },
        })
      )
  ...
  async updateProduct(product: ProductDTO): Promise<Product> {
    try {
      ...
      // Check for duplicate name only if it's different from current name
      const duplicateProduct = await this.productRepo.findOne({
        where: { seller: { id: product.seller }, name: product.name },
      });
      ...
      this.logger.error(
        `<-- Error updating product with id ${product.id}: ${error}`,
      );
      throw error;
    }
  }
  ...
  async searchByName(name: string): Promise<Product[]> {
    try {
      ...
      this.logger.error(
        `<-- Error searching for products with name ${name}: ${error}`,
      );
      throw error;
    }
  }
  ...
  async getTrendingProducts(): Promise<Product[]> {
    ...
    try {
      const trendingProductIds = (await this.productRepo.query(`
                SELECT productId 
                FROM order_items 
