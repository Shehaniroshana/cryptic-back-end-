...
export class OffersService {
  ...
  async createOffer(offers: offerDto): Promise<Offers> {
    try {
      if (
        !(await this.productRepo.findOneBy({
          id: offers.product,
          isActive: true,
        }))
      )
      ...
      if (
        await this.offerRepo.findOneBy({
          id: offers.id,
          isActive: true,
          title: offers.title,
  ...
  async getAllOffers(): Promise<offerDto[]> {
    try {
      const offersEntity = await this.offerRepo.find({
        where: { isActive: true },
      });
      const offers = plainToInstance(offerDto, offersEntity);
      return offers;
    } catch (error) {
      this.logger.error('Error getting all offers', error);
      throw error;
    }
  }
  ...
  async deleteOffer(id: string): Promise<Offers> {
    try {
      ...
      this.logger.error('Error deleting offer', error);
      throw error;
    }
  }
  ...
  async getOffersByUserIdAndProductId(
    ...
    try {
      ...
      const offers = await this.offerRepo.find({
        where: {
          product: { id: productId, seller: { id: userId, isActive: true } },
          isActive: true,
        },
      ...
      this.logger.error(
        'Error getting offers by user id and product id',
        error,
      );
      throw error;
    }
  ...
  async getOffersByProductId(productId: string): Promise<offerDto[]> {
    try {
      ...
      this.logger.error('Error getting offers by product id', error);
      throw error;
    }
  }
  ...
  async getOffersByDiscount(discount: number): Promise<offerDto[]> {
    try {
      ...
      this.logger.error('Error getting offers by discount', error);
      throw error;
    }
  }
  ...
  async getOffersByDiscountBetween(
    ...
    try {
      ...
      const offers = await this.offerRepo.find({
        where: { discount: Between(min, max), isActive: true },
        relations: ['product', 'product.seller'],
      });
      if (!offers.length) throw new BadRequestException('Offers not found');
      return plainToInstance(offerDto, offers);
    } catch (error) {
      this.logger.error('Error getting offers by discount between', error);
      throw error;
    }
  }
  ...
  async getOffersByStartDate(date: Date): Promise<offerDto[]> {
    try {
      ...
      const offers = await this.offerRepo.find({
        where: { startDate: date, isActive: true },
        relations: ['product', 'product.seller'],
      });
      if (!offers.length) throw new BadRequestException('Offers not found');
      return plainToInstance(offerDto, offers);
    } catch (error) {
      this.logger.error('Error getting offers by start date', error);
      throw error;
    }
  }
  ...
  async getOffersByEndDate(endDate: Date): Promise<offerDto[]> {
    try {
      ...
      const offers = await this.offerRepo.find({
        where: { endDate, isActive: true },
        relations: ['product', 'product.seller'],
      });
      if (!offers.length) throw new BadRequestException('Offers not found');
      return plainToInstance(offerDto, offers);
    } catch (error) {
      this.logger.error('Error getting offers by end date', error);
      throw error;
    }
  }
  ...
  async getOffersByTitle(title: string) {
    try {
      ...
      this.logger.error('Error getting offers by title', error);
      throw error;
    }
  }
  ...
  async getOffersBySellerId(sellerId: string) {
    try {
      ...
      const offers = await this.offerRepo.find({
        where: { product: { seller: { id: sellerId } }, isActive: true },
        relations: ['product', 'product.seller'],
      });
      if (!offers.length) throw new BadRequestException('Offers not found');
      return plainToInstance(offerDto, offers);
    } catch (error) {
      this.logger.error('Error getting offers by seller id', error);
      throw error;
    }
  }
  ...
  async getOffersByStartAndEndDate(startDate: Date, endDate: Date) {
    try {
      ...
      if (!endDate) throw new BadRequestException('End date is required');
      const offers = await this.offerRepo.find({
        where: { startDate: Between(startDate, endDate), isActive: true },
        relations: ['product', 'product.seller'],
      });
      if (!offers.length) throw new BadRequestException('Offers not found');
      return plainToInstance(offerDto, offers);
    } catch (error) {
      this.logger.error('Error getting offers by start and end date', error);
      throw error;
    }
  }
}
