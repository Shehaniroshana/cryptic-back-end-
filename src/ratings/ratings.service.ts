...
export class RatingsService {
    ...
    async getRatingsByProductId(id: number): Promise<Rating[]> {
        try {
            ...
            const ratings = await this.ratingRepo.find({ where: { product: { id }, isActive: true }, relations: { user: true, product: true } });
    ...
    async getRatingsByUserId(id: number): Promise<Rating[]> {
        try {
            ...
            const ratings = await this.ratingRepo.find({ where: { user: { id }, isActive: true }, relations: { user: true, product: true } });
            if (!ratings.length) throw new BadRequestException('Ratings not found');
            return plainToInstance(Rating, ratings);
        } catch (error) {
    ...
    async getRatingsByUserIdAndProductId(userId: number, productId: number): Promise<Rating[]> {
        try {
            if (!userId || !productId) throw new BadRequestException('User id and product id are required');
            const ratings = await this.ratingRepo.find({ where: { user: { id: userId }, product: { id: productId }, isActive: true }, relations: { user: true, product: true } });
            if (!ratings.length) throw new BadRequestException('Ratings not found');
            return plainToInstance(Rating, ratings);
        } catch (error) {
            this.logger.error('Error getting ratings by user id and product id', error);
            throw error;
        }
    }
